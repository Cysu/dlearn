...
class LinearCombLayer(Block):

    r"""Construct a linear combination layer.
    ...
    def __init__(self, input, input_shape,
        ...
        self._output_shape = self._input_shape[1:]
        if self._flatten:
            self._output_shape = np.prod(self._output_shape)

        # Initialize parameters
        if W is None:
            fan_in = np.prod(self._input_shape)
            fan_out = np.prod(self._output_shape)
